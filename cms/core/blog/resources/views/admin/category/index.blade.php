...
@section('body')
    <div class="table-div">
        <a class="btn btn-default btn-sm hor-align pull-right btn-right-spacing" href="{{route('blog-category.create')}}" ><i class='glyphicon glyphicon-plus-sign'></i>&nbsp;&nbsp;New</a>
...
<script>
    $('document').ready(function(){
        ...
        var url =  '{{route('get_blog-category_data_from_admin')}}';
        var column = [
            { data: 'check', name: 'check', searchable: false, sortable: false , width: '9%' , render : function(data, type, row, meta)
            {
                return '<input id="'+data+'" class="check_class" type="checkbox" value='+row["id"]+' name="selected_category[]"><label for="'+data+'"><span></span></label>';
            }
            },
        ...
        var options  = {
            ...
            button : [
                {
                    name : "Publish" ,
                    url : "{{route('blog-category_action_from_admin',1)}}"
                },
                {
                    name : "Un Publish",
                    url : "{{route('blog-category_action_from_admin',0)}}"
                },
                {
                    name : "Trash",
                    url : "{{route('blog-category_action_from_admin',-1)}}"
                ...
                {
                    name : "Delete",
                    url : "{{route('blog-category.destroy',1)}}",
                    method : "DELETE"
                }
            ],
